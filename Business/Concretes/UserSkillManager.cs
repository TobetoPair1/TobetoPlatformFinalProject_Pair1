...
namespace Business.Concretes
{
	public class UserSkillManager : IUserSkillService
	{
		...
		{
			UserSkill userSkill = await _userSkillDal.GetAsync(us => us.Id == deleteUserSkillRequest.Id);
			var deletedUserSkill = await _userSkillDal.DeleteAsync(userSkill);
		...
		{
			var result = await _userSkillDal.GetAsync(us => us.Id == getUserSkillRequest.Id, include: us => us.Include(us => us.Skill));
